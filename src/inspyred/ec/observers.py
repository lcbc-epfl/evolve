...
def stats_observer(population, num_generations, num_evaluations, args):
    ...
    .. note::
       ...
       This function makes use of the ``test.ec.analysis.fitness_statistics`` 
...
def file_observer(population, num_generations, num_evaluations, args):
    ...
    'test-statistics-file-<timestamp>.csv', contains the basic
    ...
    'test-individuals-file-<timestamp>.csv', contains every individual
    ...
    try:
        ...
        statistics_file = open('test-statistics-file-{0}.csv'.format(time.strftime('%m%d%Y-%H%M%S')), 'w')
        args['statistics_file'] = statistics_file
    try:
        individuals_file = args['individuals_file']
    except KeyError:
        individuals_file = open('test-individuals-file-{0}.csv'.format(time.strftime('%m%d%Y-%H%M%S')), 'w')
...
class EmailObserver(object):
    ...
    - *subject* -- the subject of the email (default 'test observer report')
    ...
    def __init__(self, username, password, server, port=587):
        ...
        self.subject = "test observer report"
