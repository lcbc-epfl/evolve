...
def generational_replacement(random, population, parents, offspring, args):
    ...
    popsort_id = argsort(parents)
    
    rev_popsort_id = popsort_id[::-1]
    for i in xrange(0, num_elites):
        parents[rev_popsort_id[i]].index = parents[rev_popsort_id[i]].parent_index + len(population)
    # print " ".join(str(population[i].fitness) for i in popsort_indexes[::-1])
    parents.sort(reverse=True)
    offspring.extend(parents[:num_elites])
