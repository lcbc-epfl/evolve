...
def mutator(mutate):
    """Return an test mutator function based on the given function.
    ...
    def inspyred_mutator(random, candidates, args):
        mutants = []
        discrete_mut = args.setdefault('discrete_mutation', False)
        for i, cs in enumerate(candidates):
            mutant = mutate(random, cs, args)
            for j, _ in enumerate(mutant):
                if (discrete_mut):
                    mutant[j] = round(mutant[j], 0)
...
@mutator
def polynomial_mutation(random, candidate, args):
    """Polynomial mutation as implemented in original NSGA-II algorithm in
    C by Deb.
    """
    eta = args.setdefault('eta', 7)
    mut_rate = args.setdefault('mutation_rate', 0.1)
    bounder = args['_ec'].bounder
    mutant = copy.copy(candidate)
    for i, (xl, xu) in enumerate(zip(bounder.lower_bound, bounder.upper_bound)):
        if random.random() <= mut_rate:
            try:
                x = mutant[i]
                delta_1 = (x - xl) / (xu - xl)
