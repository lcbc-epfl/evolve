...
class Individual(object):
    ...
    - *parent_index* -- parent responsible for this individual
...
class EvolutionaryComputation(object):
    ...
    - *terminator* -- the (possibly list of) terminator(s) (defaults to 
      ``default_termination``)
    - *logger* -- the logger to use (defaults to the logger 'test.ec')
    ...
    used in the ``main`` or calling scope to accomplish that::
        ...
        import logging
        logger = logging.getLogger('test.ec')
    ...
    def __init__(self, random):
        ...
        self.logger = logging.getLogger('test.ec')
        try:
    ...
    def evolve(self, generator, evaluator, pop_size=100, seeds=None, maximize=True, bounder=None, **args):
        ...
        parent_indexes = [-1 for v in initial_cs]

        self._kwargs['parent_indexes'] = parent_indexes
            
        initial_fit = evaluator(candidates=initial_cs, args=self._kwargs)
        
        print "initial"
        for i, (cs, fit) in enumerate(zip(initial_cs, initial_fit)):
            if fit is not None:
        ...
        while not self._should_terminate(list(self.population), self.num_generations, self.num_evaluations):
            ...
            # for i, v in enumerate(parents):
                # print "b1", i, v.index, v.parent_index, v.fitness
            # Need to deep copy parents as want new memory allocation such that following doesn't produce weird overwriting
            parents_copy = [copy.deepcopy(i) for i in parents]
            parent_cs = [copy.deepcopy(i.candidate) for i in parents_copy]
            offspring_cs = parent_cs
    
            print "post selection"
            for i, v in enumerate(parents_copy):
                curr = parents_copy[i]
                # print "b", i, curr.index, curr.parent_index, curr.fitness
                curr.parent_index = parents[i].index
                curr.index = i
                print i, curr.candidate, curr.index, curr.parent_index, curr.fitness
            # print "done"
            ...
            parent_indexes = [v.parent_index for v in parents_copy]

            # bad solution to problem of passing 'closest parent' from previous generation into current for evaluation
            self._kwargs['parent_indexes'] = parent_indexes
            ...
            for i, (cs, fit) in enumerate(zip(offspring_cs, offspring_fit)):
                if fit is not None:
                    off = Individual(cs, maximize=maximize)
                    off.fitness = fit
                    off.index = i
                    off.parent_index = parent_indexes[i]
            ...
            self.population = self.replacer(random=self._random, population=self.population, parents=parents_copy, offspring=offspring, args=self._kwargs)
            self.logger.debug('population size is now {0}'.format(len(self.population)))
...
class ES(EvolutionaryComputation):
    ...
    the same length (using ``test.ec.generators.strategize``). These 
